...
Los perfiles de velocidad y temperatura en $t^*=0$ son muy similares a los perfiles del flujo base laminar.  Por lo cuál, los valores de Re$_{\tau}$, Nu y $f$ calculados con la condición inicial son equivalentes a los calculados en el régimen laminar. Asimismo, los valores de TKE y la varianza de la temperatura se aproximan utilizando las autofunciones $\lbrace \widehat{v_x}, \widehat{v_y}, \widehat{v_z}, \widehat{\theta} \hspace{0.3mm} \rbrace$ que aparecen en las expresiones de las perturbaciones. Esto surge de considerar lo siguiente: 
...
\begin{figure}%[H]
  ...
  \subfloat[]{
    \includegraphics[width=0.40\textwidth]{figures/cap6/A-C10/screenshots_times/t338_zy.png}
...
Al inpeccionar la energía cinética tubulenta y la varianza de la temperatura, se distingue que en el ensayo A-C10 primero se produce un aumento y descenso de estas magnitudes (dos picos de menor intensidad) antes del crecimiento que da paso al establecimiento del nuevo estado de flujo; sin embargo, en B-C2 no existen aumentos y/o descensos intermedios de las magnitudes sino un crecimiento brusco que ocurre en un instante de tiempo temprano en comparación al ensayo A-C10. Es claro que en el ensayo B-C2, el sistema es mucho más sensible a las perturbaciones impuestas mientras que en el ensayo A-C10 necesita de la inestabilidad secundaria para desencadenar la transición. %Se remarca que, en ambos ensayos, ambas cantidades presentan estados transitorios no monótonos con máximos y mínimos que exceden los valores de la condición inicial laminar y del estado turbulento desarrollado.


%Al inspeccionar el coeficiente de fricción, el ensayo A-C10 inicia con un valor $f$ menor al que luego alcanza en el estado turbulento desarrollado pasando por un mínimo y un máximo en su evolución; por su parte, en B-C2 ocurre lo contrario, partiendo de un valor inicial mas alto, este evoluciona a un nuevo máximo para luego alcanzar en forma monóntona su valor estadísticamente estacionario. Asimismo, antes del valor pico mencionado, el coeficiente de fricción experimenta un valle en el ensayo A-C10, sin embargo, en B-C2, la magnitud no experimenta decaimiento alguno. En general, las magnitudes TKE, varianza de la temperatura y factor de fricción evolucionan al estado estadísticamente estacionario experimentando más fluctuaciones en el caso A-C10 respecto al caso B-C2.
...
%ORIGINAL
%Por último, se puede destacar que en el ensayo A-C10 el Re$_{\tau}$ parte de un valor laminar inicial, alcanza un valor máximo, y luego se estabiliza en torno a un valor superior al del estado inicial (véase cuva azul de la Figura \ref{fig:retau-comp}). En cambio, en el ensayo B-C2, el Re${\tau}$ parte de un estado laminar inicial, también crece, alcanza un máximo y posteriormente desciende de manera considerable hasta situarse por debajo del valor inicial (véase cuva verde de la Figura \ref{fig:retau-comp}). Esto se puede interpretar físicamente a partir de la definición de Re$_{\tau}$. En ella, aparece $u_{\tau}$ que a su vez depende del gradiente de $\langle u^*_x \rangle$ en la pared ($d \langle u^*_x \rangle /dy$ evaluada en $y^*=-1$). En el ensayo A-C10, si se observa el perfil de velocidad en $t^*=2$ (Figura \ref{fig:mosaico-ac10}) es posible apreciar cualitativamente que la pendiente del perfil inicial es menor la pendiente del perfil del caso turbulento desarrollado cerca de la paredes. El sistema antes de estabilizarse experimenta un incremento brusco de turbulencia, evidenciado por un crecimiento muy grande de la TKE, y esta, como se mencionó anteriormente, tiende a difundir el momento y a aplanar los perfiles (véase $t^*=750$ en Figura \ref{fig:mosaico-ac10}) y como consecuencia el gradiente de la velocidad \textit{streamwise} cerca de la pared aumenta. Una explicación análoga se da para el ensayo B-C2. Al observar el perfil de velocidad en $t^*=2$ (Figura \ref{fig:mosaico-bc2}) es posible apreciar (cualitativamente) que la pendiente del perfil inicial cerca la pared es mayor que para el perfil del caso turbulento desarrollado. El sistema previo a estabilizarse para un valor de Re$_{\tau}$ menor experimenta un crecimiento violento en la TKE en $t^* =46$ que coincide con un achatamiento muy marcado en los perfiles de velocidad (Figura \ref{fig:mosaico-bc2}) y por ende un gradiente de velocidad \textit{streamwise} menor al caso lamianr inicial.  
