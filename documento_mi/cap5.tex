...
%
%
%El propósito de este capítulo es mostrar, mediante simulaciones DNS, cómo la flotación (boyancia) modifica un flujo turbulento plenamente desarrollado en un canal de placas paralelas. Para ello se explora un amplio espectro de números de Reynolds, Richardson y Prandtl, abarcando la transición desde convección forzada hasta convección natural. Tras describir los casos y las variables adimensionales de referencia, se presentan: (i) magnitudes estadísticas de primer y segundo orden, (ii) la influencia del número de Prandtl sobre las subcapas viscosa y térmica, (iii) el número de Nusselt y (iv) el factor de fricción de Darcy.
...
%De manera sintética, los principales hallazgos que guían el capítulo son los siguientes, en secuencia. Primero, se observa que la fuerza boyante transforma los perfiles de velocidad hacia una morfología tipo “M” y desplaza el máximo de $\langle u_x^* \rangle$ hacia la pared. Luego, para flujos con $0 < \text{Ri}_b < 1$ los perfiles de temperatura adimensional se ubican por encima del caso puramente forzado y para $\text{Ri}_b>1$, la mezcla inducida por la flotación “aplana” el perfil medio adimensional de temperatura. Tercero, el efecto del número de Prandtl se refleja en la ley de pared de temperatura: con $\text{Pr}=0\text{.}071$ se verifica $\langle \theta^{*} \rangle \simeq \text{Pr} \hspace{1mm} y^+$ hasta $y^+ \approx 30$, mientras que con $\text{Pr}=0\text{.}71$ concluye en $y^+ \approx 7$, lo que evidencia la influencia de $\text{Pr}$ en la capa conductiva. Cuarto, se observa la existencia un intervalo $10^{-6} \lesssim \text{Bo} \lesssim 3\times10^{-5}$ (donde Bo es el número de boyancia \cite{jackson1989studies}) en el que $\text{Nu}$ se reduce respecto al caso puramente forzado; fuera de él, la transferencia térmica se recupera y supera a la de convección forzada. Esta caída coincide con una menor producción de turbulencia cerca de las paredes. Por último, pese a la asistencia de la boyancia, el gradiente de velocidad en la pared aumenta y eleva el factor de Darcy. Se propone una la correlación del factor de fricción en función del número de boyancia Bo y se observa que reproduce con buena fidelidad tanto los datos simulados como los de referencia.
...
\begin{figure}[H]
  \centering
  \subfloat[]{
    \includegraphics[width=0.49\textwidth]{figures/cap5/nusselt_corr/nu_corr.eps}
    \label{fig:nu_vs_bo}}
  \subfloat[]{
...
\begin{figure}[H]
  \centering
  \subfloat[]{
    \includegraphics[width=0.49\textwidth]{figures/cap5/Re5000-Pr071-Ri1Em2_shear_prod.png}
    \label{fig:shear_prod}}
  \subfloat[]{
    \includegraphics[width=0.49\textwidth]{figures/cap5/Re5000-Pr071-Ri1Em2_buoya_prod.png}
...
%En términos equivalentes, al aparecer la fuerza boyante, esta actuá atenuando la intensidad de la turbulencia en la zona cercana a la pared. Esto se traduce en un aumento de la capa conductora y el efecto de difusión turbulenta no resulta tan eficiente (véase Figura \ref{fig:phi-log-law-Re5000-Pr071}). La diferencia de temperatura adimensonal entre la pared y el centro del canal crece dando lugar a una transferencia de calor menos eficiente (Figuras  \ref{fig:phi-Re5000-Pr071}, \ref{fig:phi-wall-law-Re5000-Pr071} y \ref{fig:nu_vs_bo}). Al aumentar la fuerza boyante, comienza a aumentar la intensidad de la turbulencia y se reduce el ancho de la capa conductora (Figura \ref{fig:phi-wall-law-Re5000-Pr071}), el perfil de velocidad streamwise incrementa su intensidad en la zona cercana a la pared (Figura \ref{fig:ux-wall-law-Re5000-Pr071}), donde el fluido se encuentra con mayor temperatura y se logra una transferencia de calor superior al caso forzado. 
